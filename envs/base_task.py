    ...
    def render(self):
        ...
        if self.cfg["viewer"]["record_video"]:
            ...
            if self.camera is None:
                ...
                self.camera_frames = []
            env_idx = self.cfg["viewer"]["record_env_idx"]
            if self.root_states.dim() == 3:
                robot_pos = self.root_states[env_idx, 0, 0:3].tolist()
            else:
                robot_pos = self.root_states[env_idx, 0:3].tolist()
            cam_pos = gymapi.Vec3(
                *(x + y for x, y in zip(robot_pos, self.cfg["viewer"]["pos"]))
            )
            cam_target = gymapi.Vec3(*robot_pos)
